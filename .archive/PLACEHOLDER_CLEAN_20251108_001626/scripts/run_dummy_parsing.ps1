...
$reass = Join-Path $repo "examples\pcap\decoded\dummy_reassembled.json"
$parsed = Join-Path $repo "examples\pcap\decoded\dummy_parsed.json"
$ndj = Join-Path $repo "examples\pcap\decoded\dummy_parsed.ndjson"
...
# 5) small summary
Write-Host "`n=== Done ==="
Write-Host "Reassembled:" $reass
Write-Host "Parsed JSON :" $parsed
Write-Host "NDJSON      :" $ndj