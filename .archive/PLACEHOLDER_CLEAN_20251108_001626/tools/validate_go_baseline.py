...
SRC_B = r"examples/pcap/decoded/dummy_parsed_new.ndjson"
OUT_ND = r"examples/pcap/decoded/dummy_go_strict.ndjson"
OUT_JSON = r"examples/pcap/decoded/dummy_go_strict.json"
...
def main():
    ...
    with open(REPORT, "w", encoding="utf-8") as f:
        f.write("# Go Baseline Validation (Strict Mode)\n\n")
        f.write(f"- Input: `{src}`\n")
        f.write(f"- Output NDJSON: `{OUT_ND}`\n")
        f.write(f"- Output JSON: `{OUT_JSON}`\n\n")
        f.write(f"## Summary\n\n")
        f.write(f"- Total rows: **{total}**\n")
        f.write(f"- Rows with parse_error: **{with_errors}**\n")
        f.write(f"- Unique prefixes: **{len(by_prefix)}**\n\n")
        f.write("### By prefix (top 15)\n\n")
        for p, c in by_prefix.most_common(15):
            f.write(f"- `{p}`: {c}\n")
        f.write("\n## Shape Check Failures\n\n")
        if not errs:
            f.write("✅ No shape failures detected.\n")
        else:
            f.write(f"❌ {len(errs)} shape failures:\n")
            for i, msg in errs[:50]:
                f.write(f"- Row {i}: {msg}\n")
            if len(errs) > 50:
                f.write(f"- … (+{len(errs)-50} more)\n")
