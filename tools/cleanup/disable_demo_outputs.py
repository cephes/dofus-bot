...
PATTERNS = [
    r"\bparsed_data\b",
    r"\bsample_go\b",
# DEMO_DISABLED:     r"Hello world from Go",
...
# DEMO_DISABLED:     r"pretty json",
# DEMO_DISABLED:     r"Pretty JSON",
]

WRITE_HINTS = [
# DEMO_DISABLED:     r"json\.dump", r"json\.dumps", r"write\(", r"fprintf", r"fmt\.Fprint", r"fmt\.Fprintf",
# DEMO_DISABLED:     r"Write-Host", r"Out-File", r"Set-Content", r"Add-Content", r"ConvertTo-Json",
    r"decoded[\\/].*\.ndjson", r"decoded[\\/].*\.json",
    r"dummy_parsed_all\.ndjson", r"john_parsed_all\.ndjson",
# DEMO_DISABLED:     r"dummy_go.*\.ndjson", r"john_go.*\.ndjson"
]
...
def main():
    import argparse
# DEMO_DISABLED:     ap = argparse.ArgumentParser(description="Find and optionally disable placeholder/demo outputs across the repo.")
...
# DEMO_DISABLED:         json.dump({"root": str(ROOT), "suspects": suspects}, f, ensure_ascii=False, indent=2)

    with report_md.open("w", encoding="utf-8") as f:
# DEMO_DISABLED:         f.write("# Placeholder / Demo Writers — Suspect Report\n\n")
# DEMO_DISABLED:         f.write(f"- Repo root: `{ROOT}`\n")
# DEMO_DISABLED:         f.write(f"- Total suspects: **{len(suspects)}**\n")
# DEMO_DISABLED:         f.write(f"- Auto-touchable (in tools/ or scripts/): **{sum(1 for s in suspects if s['touchable'])}**\n\n")
        for s in suspects[:2000]:
# DEMO_DISABLED:             f.write(f"## {s['file']}  (score: {s['score']}, touchable: {s['touchable']})\n\n")
            for m in s["matches"][:30]:
# DEMO_DISABLED:                 f.write(f"- L{m['line']}: `{m['content']}` (score {m['score']})\n")
            if len(s["matches"]) > 30:
# DEMO_DISABLED:                 f.write(f"- … plus {len(s['matches']) - 30} more lines\n")
...
# DEMO_DISABLED:         print("[DRY-RUN] No changes applied. Re-run with --apply to comment out demo writers in tools/ and scripts/.")
        return
