...
if [[ -f "examples/pcap/decoded/dummy_parsed_new.ndjson" && -f "examples/pcap/decoded/dummy_go.ndjson" ]]; then
...
if [[ "$has_initial_data" == false ]]; then
    ...
    python tools/diff_go_rust_parsed.py --rust examples/pcap/decoded/dummy_parsed_new.ndjson --go examples/pcap/decoded/dummy_go.ndjson --json-out go_rust_diff.json --md-out GO_RUST_DIFF.md || { echo "ERROR: Initial diff failed"; exit 1; }
fi
...
echo "  Running diff..."
python tools/diff_go_rust_parsed.py --rust examples/pcap/decoded/dummy_parsed_new.ndjson --go examples/pcap/decoded/dummy_go.ndjson --json-out go_rust_diff.json --md-out GO_RUST_DIFF.md || { echo "ERROR: Diff failed"; exit 1; }
